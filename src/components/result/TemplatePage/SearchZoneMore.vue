<template>
  <div class="search-zone-more-wrap">
    <template v-for="item in searchZoneList">
      <div
        :data-value="nowCategoryCode"
        :data-code="nowTemplateId"
        :key="item.categoryCode"
        v-if="searchZoneList && item.subList && item.subList.length > 0"
      >
        <template v-for="(itemSub, indexSub) in item.subList">
          <div
            :key="indexSub.categoryCode"
            v-if="
              nowTemplateId == itemSub.verticalTemplateId &&
              itemSub.searchZoneData &&
              itemSub.searchZoneData.length > 0
            "
          >
            <div
              :class="
                itemSub.searchZoneData.length == 1 &&
                itemSub.searchZoneData[0].pCode == 'P00000057388' &&
            ...
            >
              ...
              >
                <ul>
                  <li
                    ...
                    v-for="(itemD, indexD) in itemSub.searchZoneData"
                    :key="itemD._id"
              ...
              <div class="m-content" v-else>
                <div
                  class="m-c-top"
                  v-html="
                    itemSub.searchZoneData[currentIndex].titleHL
                      ? itemSub.searchZoneData[currentIndex].titleHL
                      : itemSub.searchZoneData[currentIndex].title
                  "
                ></div>
                <van-swipe
                  :autoplay="3000"
                  :class="
                    itemSub.subTitleFlag
                      ? 'height208'
                      : !itemSub.subTitleFlag &&
                        itemSub.searchZoneData &&
                        itemSub.searchZoneData.length == 1
                      ...
                      : itemSub.eightFlag &&
                        itemSub.searchZoneData &&
                        itemSub.searchZoneData.length - 1 == 1
                      ? 'height225'
                      : 'height245'
                  "
                ...
                >
                  <template v-for="(itemD, indexD) in itemSub.searchZoneData">
                    <van-swipe-item
                      :key="itemD._id"
                      :class="
                        itemSub.subTitleFlag
        ...
        this.$nextTick(() => {
          ...
          elements.forEach((elementE, indexE) => {
            ...
            this.searchZoneList.forEach((elementF, indexF) => {
              elementF.subList &&
                elementF.subList.forEach((elementSub, indexSub) => {
                  elementSub.searchZoneData &&
                    elementSub.searchZoneData.forEach(
                      (elementData, elementindex) => {
                        // 判断当前元素是否是需要的元素
                        if (obj.WT_markId == elementData._id) {
                          // console.log(JSON.stringify(elementE.impAttrsList),'-------------searchZone-搜索结果页_搜索专区----------------');
                          // if(isApp()){
                          ...
                          // elementData.impAttrsList.WT_area_type_2 =
                          //   obj.categoryName;
                          // elementData.impAttrsList.WT_area_name =
                          //   obj.categoryName;
  ...
  methods: {
    ...
    async handleClickTo(categoryName, item, index) {
      ...
      console.log(
        ...
        "-------------searchZoneMore-点击商品-点击事件传参----------------"
      );
      console.log("-----searchZoneMore-商品position-----" + (index + 1));
      /**
       * 埋点-插码-上报
       * 点击事件采集
       * 搜索结果页_搜索专区
      ...
      const clickGdpObj = {
        ...
        WT_area_name: item.wt_area_name, // 若没有二级分类，此名称则记录为一级分类名称 // 这里应该是商品_手机硬件类传参 一级分类名称暂时无法透传过来 先搁置
        XY_env_type: "icon",
        XY_gd_source: "搜索平台",
