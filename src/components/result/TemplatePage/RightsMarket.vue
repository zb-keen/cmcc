    ...
    <div class="module-wrap">
      ...
      <div class="m-content" @touchmove.stop>
        <ul class="m-content-ul">
          <li
            class="content-item"
            v-for="(itemD, indexD) in arrRightsList"
            :key="indexD"
          ...
          >
            ...
            <div
              v-if="
                isFieldToShow('subTitle', itemD.verticalTemplateFieldList) &&
...
export default {
  ...
  watch: {
    ...
    arrRightsList: {
      handler(newVal) {
        console.log("垂直页--频道服务类数据曝光====>");
        console.log(newVal);
        if (newVal && newVal.length) {
          newVal.map((channelItem) => {
            let serverArr = null;
            ...
            if (serverArr && serverArr.length) {
              serverArr.map((serverItem, serverIndex) => {
                const impGdpObj = {
                  ...
                  WT_area_type_2: channelItem.parentCategoryName,
                  WT_area_name: `${channelItem.dispCategoryName}_${channelItem.title}`, // 若没有二级分类，此名称则记录为一级分类名称
                  XY_env_type: "icon",
                ...
                console.log(
                  "XRightsMarket垂直页-权益超市-服务类数据曝光上报===>"
                );
  ...
  methods: {
    ...
    handleTouchMove(event) {
      // 这里的代码会在滑动时执行，并且阻止事件冒泡
      console.log("滑动事件触发，且阻止了冒泡");
    },
    async handleClickTo(item, index) {
      ...
      console.log(
        ...
        "-------------垂直页RightsMarket-点击商品-点击事件传参----------------"
      );
      console.log("-----垂直页RightsMarket-商品position-----" + posIndex);
      ...
      const clickGdpObj = {
        ...
        XY_first_class: item.parentCategoryName,
        ...
        areaid: "", // 父物品id信息（无父物品传空）
        WT_area_type_2: "生活",
        WT_area_name: "惠民", // 若没有二级分类，此名称则记录为一级分类名称
        XY_env_type: "icon",
        WT_event: "P00000050506", // 全部页传P00000050508 垂直页传P00000050506
        XY_gd_source: "搜索平台",
        XY_scene: this.getSearchContent() || "", // 搜索内容
      ...
      console.log("点击垂直页-权益超市的-频道-数据上报===>");
    ...
    async directGoPage(item, index, itemParentObj) {
      ...
      console.log("点击垂直页-权益超市的-服务-数据上报===>");
      console.log(clickGdpObj);
...
.rights-content-x {
  background: #fff;
  border-radius: 0.053333rem !important;
}
